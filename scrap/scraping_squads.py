"""
Scraping de planteles/squads deportivos
======================================

Este módulo se encarga de descargar información de planteles de equipos deportivos
desde la API de ScoresWay, incluyendo datos detallados de jugadores, cuerpo técnico
y información de equipos organizados por directorios de competición y temporada.
...
class SquadsScraper:
    """
    Clase para hacer scraping de planteles/squads desde la API de ScoresWay.
    """
    ...
    def __init__(self, 
                 ...
                 data_dir: str = 'data'):
        """
        Inicializa el scraper de planteles.
    ...
    def set_pagination_config(self, page_size: int = 50, page_number: int = 1, detailed: bool = True) -> None:
        """
        Configura los parámetros de paginación para la API.
        
        Args:
            page_size (int): Número de elementos por página
            page_number (int): Número de página a obtener
            detailed (bool): Si obtener información detallada
        """
        self.page_size = page_size
        self.page_number = page_number
        self.detailed = detailed
        print(f"📄 Paginación configurada: {page_size} elementos/página, página {page_number}, detallado: {detailed}")
    
    def obtener_squads_json(self, torneo_id: str, competicion_name: str, referer: str = None) -> Dict:
        """
        Obtiene los datos de planteles desde la API.
        ...
        Returns:
            Dict: Datos de planteles en formato JSON
        ...
        try:
            ...
            # Construir URL de la API con parámetros de paginación
            squads_url = (
                f"{self.api_base_url}/{self.sdapi_outlet_key}/"
                f"?_rt=c&tmcl={torneo_id}&_pgSz={self.page_size}&_pgNm={self.page_number}"
            ...
            print(f"👥 API URL: {squads_url}")
            
            # Realizar petición
            response = self.session.get(squads_url, headers=headers)
    ...
    def save_squads_json(self, season_row: pd.Series, skip_existing: bool = True) -> bool:
        """
        Descarga y guarda el squads JSON para una temporada.
        ...
        try:
            ...
            # Obtener datos de planteles
            squads_data = self.obtener_squads_json(
                torneo_id=torneo_id,
            ...
            print(f"✅ Squads guardado: {json_path}")
            self.exitos += 1
    ...
    def process_seasons(self, 
        ...
        """
        Procesa múltiples temporadas para descargar planteles.
        ...
        try:
            ...
            print(f"🚀 Iniciando descarga de planteles...")
            ...
            print(f"   - Configuración: {self.page_size} elementos/página, detallado: {self.detailed}")
            ...
            for idx, (_, row) in enumerate(df_to_process.iterrows()):
                try:
                    ...
                    # Intentar guardar planteles
                    self.save_squads_json(row, skip_existing)
                    self.procesados += 1
    ...
    def _print_final_summary(self, stats: Dict) -> None:
        ...
        """
        print(f"\n" + "="*60)
        print(f"👥 RESUMEN FINAL - DESCARGA DE PLANTELES")
        print(f"="*60)
...
def find_squads_resume_index(df_seasons: pd.DataFrame, filters: Optional[Dict] = None) -> int:
    """
    Encuentra el índice desde donde continuar basándose en planteles ya descargados.
    ...
    try:
        ...
        # Revisar qué planteles ya existen
        for idx, row in df_filtered.iterrows():
            try:
                ...
                if not os.path.exists(json_path):
                    print(f"🔍 Primer plantel faltante encontrado en índice {idx}")
        ...
        print("✅ Todos los planteles ya fueron descargados")
...
def download_squads_by_filters(df_seasons: pd.DataFrame,
                              ...
                              **scraper_kwargs) -> Dict:
    """
    Descarga planteles aplicando filtros comunes.
    ...
    Args:
        ...
        limit (Optional[int]): Límite de temporadas a procesar
        page_size (int): Número de elementos por página de la API
        detailed (bool): Si obtener información detallada
        **scraper_kwargs: Argumentos adicionales para SquadsScraper
    ...
    scraper = SquadsScraper(**scraper_kwargs)
...
def smart_download_squads(df_seasons: pd.DataFrame,
                         continente: Optional[str] = None,
                         pais: Optional[str] = None,
                         competicion: Optional[str] = None,
                         ...
                         **scraper_kwargs) -> Dict:
    """
    Función inteligente que detecta automáticamente desde dónde continuar la descarga de planteles.
    ...
    Args:
        ...
        pais (str, optional): Filtrar por país
        competicion (str, optional): Filtrar por competición
        restart_from_zero (bool): Si True, borra planteles existentes y empieza de cero
        batch_size (int): Número de temporadas a procesar en este lote
        page_size (int): Número de elementos por página de la API
        detailed (bool): Si obtener información detallada de jugadores
        **scraper_kwargs: Argumentos adicionales para SquadsScraper
    ...
    Example:
        ...
        stats = smart_download_squads(df_seasons, pais='Argentina', restart_from_zero=True)
        
        # Para continuar automáticamente:
        stats = smart_download_squads(df_seasons, pais='Argentina', restart_from_zero=False)
    """
    # Crear filtros
    filters = {}
    if continente:
    ...
    print(f"📄 Configuración API: {page_size} elementos/página, detallado: {detailed}")
    
    # Si restart_from_zero, borrar archivos existentes
    if restart_from_zero:
        deleted_count = 0
        print("🔥 Modo reinicio: Borrando planteles existentes...")
        ...
        print(f"🗑️  Eliminados {deleted_count} planteles existentes")
        start_index = 0
    else:
        # Encontrar desde dónde continuar
        start_index = find_squads_resume_index(df_seasons, filters)
        
        if start_index >= len(df_filtered):
            print("✅ Todos los planteles ya están descargados")
            return {
...
# Función de testing
def test_squads_scraper():
    """
    Función de prueba para el scraper de planteles.
    """
    print("=== Testing Squads Scraper ===")
    ...
    try:
        ...
        stats = download_squads_by_filters(
            ...
            skip_existing=True,
            limit=2,
            page_size=25,  # Menos elementos para prueba
            detailed=True
        )
