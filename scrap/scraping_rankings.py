"""
Scraping de rankings deportivos
==============================

Este módulo se encarga de descargar rankings y clasificaciones deportivas
desde la API de ScoresWay, organizando los archivos JSON por directorios
de competición y temporada. Los rankings pueden incluir clasificaciones
especiales, rankings históricos, o métricas adicionales de rendimiento.
...
class RankingsScraper:
    """
    Clase para hacer scraping de rankings deportivos desde la API de ScoresWay.
    """
    ...
    def __init__(self, 
                 ...
                 data_dir: str = 'data'):
        """
        Inicializa el scraper de rankings.
        ...
        self.api_base_url = "https://api.performfeeds.com/soccerdata/rankings"
    ...
    def obtener_rankings_json(self, torneo_id: str, competicion_name: str, referer: str = None) -> Dict:
        """
        Obtiene los datos de rankings desde la API.
        ...
        Returns:
            Dict: Datos de rankings en formato JSON
        ...
        try:
            ...
            print(f"🏅 API URL: {rankings_url}")
            
            # Realizar petición
            response = self.session.get(rankings_url, headers=headers)
            ...
            rankings_data = json.loads(content[json_start:json_end])
    ...
    def save_rankings_json(self, season_row: pd.Series, skip_existing: bool = True) -> bool:
        """
        Descarga y guarda el rankings JSON para una temporada.
        ...
        try:
            ...
            json_path = os.path.join(dir_path, 'rankings.json')
            ...
            rankings_data = self.obtener_rankings_json(
                torneo_id=torneo_id,
            ...
            print(f"✅ Rankings guardado: {json_path}")
            self.exitos += 1
    ...
    def process_seasons(self, 
        ...
        """
        Procesa múltiples temporadas para descargar rankings.
        ...
        try:
            ...
            print(f"🚀 Iniciando descarga de rankings...")
            ...
            for idx, (_, row) in enumerate(df_to_process.iterrows()):
                try:
                    ...
                    # Intentar guardar rankings
                    self.save_rankings_json(row, skip_existing)
                    self.procesados += 1
    ...
    def _print_final_summary(self, stats: Dict) -> None:
        ...
        """
        print(f"\n" + "="*60)
        print(f"🏅 RESUMEN FINAL - DESCARGA DE RANKINGS")
        print(f"="*60)
...
def find_rankings_resume_index(df_seasons: pd.DataFrame, filters: Optional[Dict] = None) -> int:
    """
    Encuentra el índice desde donde continuar basándose en rankings ya descargados.
    ...
    try:
        ...
        # Revisar qué rankings ya existen
        for idx, row in df_filtered.iterrows():
            try:
                ...
                if not os.path.exists(json_path):
                    print(f"🔍 Primer rankings faltante encontrado en índice {idx}")
        ...
        print("✅ Todos los rankings ya fueron descargados")
...
def download_rankings_by_filters(df_seasons: pd.DataFrame,
                                ...
                                **scraper_kwargs) -> Dict:
    """
    Descarga rankings aplicando filtros comunes.
    ...
    Args:
        ...
        limit (Optional[int]): Límite de temporadas a procesar
        **scraper_kwargs: Argumentos adicionales para RankingsScraper
    ...
    scraper = RankingsScraper(**scraper_kwargs)
...
def smart_download_rankings(df_seasons: pd.DataFrame,
                           continente: Optional[str] = None,
                           pais: Optional[str] = None,
                           competicion: Optional[str] = None,
                           ...
                           **scraper_kwargs) -> Dict:
    """
    Función inteligente que detecta automáticamente desde dónde continuar la descarga de rankings.
    ...
    Args:
        ...
        pais (str, optional): Filtrar por país
        competicion (str, optional): Filtrar por competición
        restart_from_zero (bool): Si True, borra rankings existentes y empieza de cero
    ...
    Example:
        ...
        stats = smart_download_rankings(df_seasons, pais='Argentina', restart_from_zero=True)
        
        # Para continuar automáticamente:
        stats = smart_download_rankings(df_seasons, pais='Argentina', restart_from_zero=False)
    """
    # Crear filtros
    filters = {}
    if continente:
    ...
    if restart_from_zero:
        deleted_count = 0
        print("🔥 Modo reinicio: Borrando rankings existentes...")
        ...
        print(f"🗑️  Eliminados {deleted_count} rankings existentes")
        start_index = 0
    else:
        # Encontrar desde dónde continuar
        start_index = find_rankings_resume_index(df_seasons, filters)
        
        if start_index >= len(df_filtered):
            print("✅ Todos los rankings ya están descargados")
            return {
...
# Función de testing
def test_rankings_scraper():
    """
    Función de prueba para el scraper de rankings.
    """
    print("=== Testing Rankings Scraper ===")
